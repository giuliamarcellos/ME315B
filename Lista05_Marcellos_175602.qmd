...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES + (ACRES * 0.20) AS ACRES20 FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, (ACRES/3) AS ACRES3 FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50)/2 AS AJUSTE FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS CTFEE FROM PRESERVE
WHERE NOT FEE = 0
```
...
```{sql connection=con}
SELECT COUNT(DNO) AS CTDNO FROM EMPLOYEE
WHERE DNO = 20
```
